        ...
        if payload.result and isinstance(payload.result, dict):
            for pattern in self.__patterns:
                for key in payload.result:
                    ...
                    elif isinstance(payload.result[key], list):
                        for i in range(len(payload.result[key])):
                            if isinstance(payload.result[key][i], dict):
                                for k in payload.result[key][i]:
                                    if isinstance(payload.result[key][i][k], str):
                                        value = re.sub(pattern[0], pattern[1], payload.result[key][i][k])
                                        payload.result[key][i][k] = value
